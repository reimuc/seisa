...
log "[refresh-ipset.sh]: 脚本启动, 刷新间隔为 ${INTERVAL} 秒"

if [ ! -x "$START_RULES" ]; then
  log "[refresh-ipset.sh]: 规则脚本 $(basename "$START_RULES") 未找到, 无法执行"
  exit 0
fi
...
refresh_ipset_loop() {
  while true; do
    log "[refresh-ipset.sh]: 执行刷新中..."
    # 执行刷新命令, 并将输出追加到主日志文件
    sh "$START_RULES" refresh >> "$LOGFILE" 2>&1 || log "[refresh-ipset.sh]: 刷新操作失败"
    # 等待指定的间隔时间
    sleep "$INTERVAL"
  done
}

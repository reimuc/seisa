  ...
  if [ "$en" = "1" ]; then
    ...
    if ! ps | awk -v monitor="$monitor" '$0 ~ monitor && !/awk/ {exit 1}'; then
      if [ -x "$MODDIR/$monitor" ]; then
        ...
        log "⚠️ 守护进程脚本未找到, 跳过启动"
      fi
    else
      log "ℹ️ 守护进程 $monitor 已在运行"
    fi
  else
    log "ℹ️ 根据配置, 守护进程已被禁用"
  fi
}
...
start_refresh_if_needed() {
  ...
  if [ "$en" = "1" ]; then
    if ! ps | awk -v refresh="$refresh" '$0 ~ refresh && !/awk/ {exit 1}'; then
      ...
      log "ℹ️ IPSet 刷新脚本 $refresh 已在运行"
    fi
  else
    log "ℹ️ 根据配置, IPSet 刷新已被禁用"
  fi
}

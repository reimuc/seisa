...
ensure_bin() {
  ...
  if [ ! -x "$MODDIR/$update" ]; then
    if [ ! -x "$BIN_PATH" ]; then
      log "❌ [service.sh]: 代理核心和更新脚本均未找到, 无法继续"
      return 1
    fi
  ...
  if [ ! -x "$BIN_PATH" ]; then
    log "❌ [service.sh]: 代理核心未找到, 请确认程序是否存在"
    return 1
  fi
  return 0
}
...
start_bin() {
  if [ ! -f "$CONFIG" ]; then
    log "❌ [service.sh]: 配置文件未找到: $CONFIG"
    return 1
  fi
  log "🚀 [service.sh]: 正在启动核心进程..."
...
apply_rules() {
  if [ -x "$START_RULES" ]; then
    log "🔥 [service.sh]: 正在应用防火墙规则..."
    sh "$START_RULES" start >> "$LOGFILE" 2>&1 || {
      log "- ❌ 规则脚本调用失败"
      return 1
    }
  else
    log "❌ [service.sh]: 规则脚本未找到, 请重新安装模块"
    return 1
  fi
  log "✅ [service.sh]: 防火墙规则应用成功"
  return 0
}
  ...
  if [ "$en" = "1" ]; then
    ...
    if ! ps | awk -v monitor="$monitor" '$0 ~ monitor && !/awk/ {exit 1}'; then
      if [ -x "$MODDIR/$monitor" ]; then
        log "- 👁️ [service.sh]: 正在启动守护进程..."
        bg_run sh "$MODDIR/$monitor"
      else
        log "- ⚠️ [service.sh]: 守护进程脚本未找到, 跳过启动"
      fi
    else
      log "- ℹ️ [service.sh]: 守护进程 $monitor 已在运行"
    fi
  else
    log "- ℹ️ [service.sh]: 根据配置, 守护进程已被禁用"
  fi
}
  ...
  if [ "$en" = "1" ]; then
    if ! ps | awk -v refresh="$refresh" '$0 ~ refresh && !/awk/ {exit 1}'; then
      if [ -x "$MODDIR/$refresh" ]; then
        log "- 🔄 [service.sh]: 正在启动 IPSet 刷新脚本..."
        bg_run sh "$MODDIR/$refresh"
      else
        log "- ⚠️ [service.sh]: 刷新脚本未找到, 跳过启动"
      fi
    else
      log "- ℹ️ [service.sh]: IPSet 刷新脚本 $refresh 已在运行"
    fi
  else
    log "- ℹ️ [service.sh]: 根据配置, IPSet 刷新已被禁用"
  fi
}
...
case "$1" in
  stop)
    log "🛑 [service.sh]: 开始执行清理..."
    cleanup
    rm -f "$FLAG" 2>/dev/null || true
    update_desc "⛔"
    log "✅ [service.sh]: 服务已停止"
    exit 0
    ;;
  *)
    log "🚀 [service.sh]: 服务启动..."
    ...
    # --- 锁机制: 防止多个实例同时运行 ---
    if [ -f "$LOCK_FILE" ]; then
      log "- ⚠️ [service.sh]: 检测到另一个服务实例正在运行, 本次启动中止"
      exit 1
    fi
    ...
    # 2. 确保核心程序存在
    if ! ensure_bin; then
      log "- ❌ [service.sh]: 代理核心不可用, 启动中止"
      exit 1
    fi

    # 3. 应用防火墙规则
    if ! apply_rules; then
      log "- ❌ [service.sh]: 防火墙规则应用失败, 启动中止"
      cleanup # 尝试清理失败的规则
      exit 1
    fi

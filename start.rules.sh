...
if [ -n "$TPROXY_PORT_FROM_CONFIG" ]; then
  log "- ⚙️ [start.rules.sh]: 检测到 TProxy 端口: $TPROXY_PORT_FROM_CONFIG"
  TPROXY_PORT=$TPROXY_PORT_FROM_CONFIG
else
  log "- ⚠️ [start.rules.sh]: 未检测到 TProxy 端口, 使用默认值: $TPROXY_PORT"
fi
...
create_ipsets() {
  log "- 📦 [start.rules.sh]: 正在创建 ipSet 集合..."
  if command -v ipset >/dev/null 2>&1; then
    ...
    log "- ⚠️ [start.rules.sh]: ipSet 命令不可用, 性能可能会受影响"
  fi
}
...
flush_ipsets() {
  log "- 🗑️ [start.rules.sh]: 正在清空 ipSet 集合..."
  if command -v ipset >/dev/null 2>&1; then
...
populate_outbound_ipsets() {
  log "- ➕ [start.rules.sh]: 正在填充出站服务器 IP 到 ipSet 集合..."
  # 解析配置文件中的出站服务器地址并添加到 ipset
  if [ -f "$CONFIG" ]; then
    # 提取所有 "server" 字段的值, 去重
    # 使用多行 awk 命令, 提高可读性, 并使用 [[:space:]] 增强兼容性
    awk '
      BEGIN {
        in_outbounds = 0
        is_proxy_type = 0
      ...
      # 匹配代理类型
      in_outbounds && /"type":[[:space:]]*"(vmess|vless|trojan|ss|ssr|shadowsocks)"/ {
        is_proxy_type = 1
      }
      # 匹配并提取 "server" 的值
      in_outbounds && /"server":/ {
      ...
      # 匹配一个 JSON 对象的结束, 如果是代理类型则打印服务器地址
      in_outbounds && /^[[:space:]]*\}/ {
        if (is_proxy_type && server != "") {
          print server
        }
        is_proxy_type = 0
        server = ""
    ...
    ' "$CONFIG" | sort -u | while read -r host; do
      log "- 🔍 [start.rules.sh]: 正在处理出站服务器: $host"
      [ -z "$host" ] && continue
...
# 创建自定义的 iptables 链
create_chains() {
  log "- 🔗 [start.rules.sh]: 正在创建自定义 iptables 链..."
  # 为 IPv4 创建自定义链
...
add_whitelists_and_rules() {
    # --- 白名单规则 (RETURN) ---
    log "- 🛡️ [start.rules.sh]: 正在添加白名单规则..."
    ...
    # 跳过代理进程自身产生的流量
    if [ -n "$PROXY_UID" ]; then
        log "  [OUT] 👤 添加代理 UID ($PROXY_UID) 白名单..."
    ...
    if [ -n "$WHITELIST_APPS" ]; then
        if command -v dumpsys >/dev/null 2>&1; then
            ...
            log "  [OUT] ⚠️ [警告] dumpsys 命令不可用, 无法处理应用白名单。"
        fi
    fi

    # --- DNS 重定向规则 (关键修复) ---
    log "- 🌐 [start.rules.sh]: 正在添加 DNS 重定向规则..."
    ...
    # --- 核心 TPROXY 规则 ---
    log "- 🔥 [start.rules.sh]: 正在添加核心 TPROXY 规则..."
...
get_proxy_uid() {
    ...
    if [ -n "$PROXY_UID" ]; then
        log "- ℹ️ [start.rules.sh]: 使用来自 settings.conf 的代理 UID '$PROXY_UID'。"
        return 0
    fi
    ...
    if [ -n "$_pid" ]; then
        ...
        log "- ⚠️ [start.rules.sh]: 从运行中的进程检测到代理 UID '$PROXY_UID'。请考虑在 settings.conf 中进行设置。"
        return
    fi

    # 3. 严重失败。
    log "❌ [start.rules.sh]: 致命错误：无法确定代理 UID。"
    log "- ➡️ [start.rules.sh]: 请在设置中将 PROXY_UID 设置为代理二进制文件 ($BIN_NAME) 运行所使用的 UID。"
    log "- ➡️ [start.rules.sh]: 例如：PROXY_UID=2000 (对于 shell 用户)"
